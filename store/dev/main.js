...
"-webkit-filter":"blur(6px)"});
	  ...
	  $("div.menu,div.news").css({'filter':"blur(8px)"})
	  $("div.outer,i.infoqtd").hide();
	  if(appdesc == "" || appdesc == "."){appdesc=appname}else {appdesc=$.base64.decode(appdesc);};
	  $("i.appdesc").html(appdesc);
	  $('div.iteminfo').show();
	  var oper='',nmbtins='';hvcontent=$('div.pkg_disp.'+appname+' span').html();
	  if(hvcontent == "")
	  ...
	  $('div.iteminfo a.openapp').attr({'href':'aio:run '+appname})
	  $('div.iteminfo a.updtapp').attr({'href':'aio:upd '+appname})
...
//Prateleira(MENU)
  //$("article").html("<div style='width:84%;position:relative'><div class='poster'></div><span id='posterleft'><</span><span id='posterright'>></span><div class='amostra'><div class='pdthumbs'><div><i class='appdesc'></i></div><div><a></a><a></a><a></a><a></a></div></div></div><div class='menu'>"+acorpo+"</div></div><div class='news'></div>");acorpo="";
//  CkInstall();
//  MenuCatIT();
...
//Seleção de Item
  $("button.app").click(function(){appname=$(this).attr('alt');appicon=$(this).attr('icon');appbg=$(this).attr('background');
	  appimg1=$(this).attr('img1');appimg2=$(this).attr('img2');appimg3=$(this).attr('img3');appimg4=$(this).attr('img4');
	  appdesc=$(this).attr('comment');GetTItem()});
//Exibir Imagens
...
//function googleTranslateElementInit() { new google.translate.TranslateElement({ pageLanguage: 'pt-br', autoDisplay: false,layout: google.translate.TranslateElement.InlineLayout.SIMPLE}, 'google_translate_element');}
